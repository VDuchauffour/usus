...
impl BillingPeriod {
    pub fn current(sub_day: u32) -> BillingPeriod {
        let now = Local::now();
        BillingPeriod::compute(
            now.day() as i32,
            now.month() as i32,
            now.year(),
            sub_day as i32,
    ...
    pub fn compute(now_day: i32, now_month: i32, now_year: i32, sub_day: i32) -> BillingPeriod {
        if now_day < sub_day {
...
mod tests {
    ...
    fn current_month_only_when_past_sub_day() {
        let p = BillingPeriod::compute(20, 5, 2026, 15);
    ...
    fn fetches_previous_month_when_before_sub_day() {
        let p = BillingPeriod::compute(10, 5, 2026, 15);
    ...
    fn january_before_sub_day_wraps_to_previous_december() {
        let p = BillingPeriod::compute(5, 1, 2026, 20);
    ...
    fn december_past_sub_day_wraps_end_to_next_january() {
        let p = BillingPeriod::compute(25, 12, 2026, 20);
    ...
    fn boundary_now_day_equals_sub_day_uses_current_month_only() {
        let p = BillingPeriod::compute(15, 5, 2026, 15);
        assert_eq!(p.months_to_fetch, vec![(2026, 5)]);
        assert_eq!(p.end, "15 Jun 2026");
        assert_eq!(p.start, NaiveDate::from_ymd_opt(2026, 5, 15).unwrap());
    }
    ...
    fn sub_day_31_in_february_clamps_to_feb_28() {
        let p = BillingPeriod::compute(15, 2, 2026, 31);
    ...
    fn sub_day_31_after_february_clamps_start_to_feb_28() {
        let p = BillingPeriod::compute(28, 3, 2026, 31);
